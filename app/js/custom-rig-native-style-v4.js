...
(function () {
  ...
  var VERSION = 'custom-rig-native-style-v4-20260710';
  ...
  function log() {
    try { console.log.apply(console, ['[PMA Custom Rig Native Style v4]'].concat([].slice.call(arguments))); } catch (e) {}
  }
  ...
  function groupNativeControllers() {
    ...
    state.scene.traverse(function (obj) {
      ...
      // v4 important: do NOT create duplicate points on the built-in original model.
  ...
  function pickTargetRig() {
    ...
    // If the app has a custom rig selected, prefer it.
    ...
    // Otherwise target the latest custom rig only.
  ...
  function createPointFor(ctrl, idx) {
    ...
    var geo = null;
    try {
      // Native-style: dodeca point, smaller than test points.
  ...
  function shouldShowPoints() {
    ...
    // After custom rig load, show points by default until user deselects.
    return true;
  }
  ...
  function onPointerDown(e) {
    ...
    if (picked) {
      // If same point is already selected, let TransformControls receive axis clicks.
    ...
    // Empty click: mimic original behavior by hiding custom points once user leaves the model.
    if (state.selectedBone || shouldShowPoints()) {
  ...
  function boot() {
    log('ready', VERSION);
    log('v4 fixes: only custom rig points, native controller colors, auto-hide on deselect. T=translate, R=rotate, S=scale, Esc=deselect.');
    requestAnimationFrame(loop);
  }
