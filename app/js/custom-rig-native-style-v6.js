...
(function () {
  ...
  var VERSION = 'custom-rig-native-style-v6-20260710';
  ...
  function log() {
    try { console.log.apply(console, ['[PMA Custom Rig Native Style v6]'].concat([].slice.call(arguments))); } catch (e) {}
  }
  ...
  function hideNativeCustomController(ctrl) {
    try {
      // v6 fix:
      // The original custom-rig BoneController meshes are children of scaled bones.
      // When PMA makes them visible during transform, they become huge green polygons.
      // We keep those original custom-rig controller meshes hidden and use our
      // scene-level native-style points for interaction instead.
      ctrl.visible = false;
      ctrl.frustumCulled = true;
      if (ctrl.material) {
        ctrl.material.opacity = 0;
  ...
  function hideAllNativeCustomControllers() {
    if (!state.scene) return;

    try {
      state.scene.traverse(function (obj) {
        if (!isBoneController(obj)) return;
        if (isCustomRigModel(controllerModel(obj))) hideNativeCustomController(obj);
      });
    } catch (e) {}
  }
  ...
  function groupNativeControllers() {
    ...
    state.scene.traverse(function (obj) {
      ...
      hideNativeCustomController(obj);
  ...
  function updatePoints() {
    hideAllNativeCustomControllers();
  ...
  function boot() {
    log('ready', VERSION);
    log('v6 fix: hides original scaled custom-rig BoneController meshes every frame, so huge green polygons should not appear. T=translate, R=rotate, S=scale, Esc=deselect.');
    requestAnimationFrame(loop);
  }
  ...
  window.PMACustomRigNative = {
    ...
    hideNativeControllers: hideAllNativeCustomControllers,
