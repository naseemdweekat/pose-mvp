...
(function () {
  ...
  var VERSION = 'custom-rig-native-style-v8-human-native-compatible-20260710';
  ...
  function log() {
    try { console.log.apply(console, ['[PMA Custom Rig Native Style v8]'].concat([].slice.call(arguments))); } catch (e) {}
  }
  ...
  function isCustomRigModel(model) {
    try {
      if (model && model.modelConst && model.modelConst.isHumanNativeRig) return false;
      return !!(model && (
        model.isCustomRig ||
  ...
  function suppressHugeNativeCustomController(ctrl) {
    try {
      // v7 fix:
      // v5 controls worked, but PMA sometimes re-showed the original custom-rig
      // BoneController meshes. Because they are children of bones scaled by ~100,
      // they appeared as huge green shapes.
      //
      // Do NOT set visible=false; v6 did that and interfered with the original
      // TransformControls behavior. Instead keep them technically present, but
      // shrink and fade them so they cannot be seen.
      if (ctrl.scale && ctrl.scale.set) ctrl.scale.set(0.001, 0.001, 0.001);
      ctrl.frustumCulled = true;
      if (ctrl.material) {
        ctrl.material.transparent = true;
        ctrl.material.opacity = 0.001;
  ...
  function suppressAllHugeNativeCustomControllers() {
    if (!state.scene) return;

    try {
      state.scene.traverse(function (obj) {
        if (!isBoneController(obj)) return;
        if (isCustomRigModel(controllerModel(obj))) suppressHugeNativeCustomController(obj);
      });
    } catch (e) {}
  }
  ...
  function groupNativeControllers() {
    ...
    state.scene.traverse(function (obj) {
      ...
      suppressHugeNativeCustomController(obj);
  ...
  function updatePoints() {
    suppressAllHugeNativeCustomControllers();
  ...
  function onPointerDown(e) {
    ...
    // v8 fix:
    // Non-point click after selecting a bone can be either:
    // 1) TransformControls axis click -> keep points/axes visible
    // 2) empty/camera click -> hide points like original app
    // Let original TransformControls process pointerdown first, then check axis state.
    if (state.selectedBone) {
      setTimeout(function () {
        try {
          var tc = state.transformControl;
          var isUsingAxis = !!(tc && (tc.dragging || tc.axis));
          if (!isUsingAxis) deselectCustomRig(true);
        } catch (e) {
    ...
    // Empty click when no custom bone is selected hides custom points.
  ...
  function boot() {
    log('ready', VERSION);
    log('v8 fix: keeps axes behavior, shrinks/fades huge native controllers, and hides points on empty click while preserving axis drag. T=translate, R=rotate, S=scale, Esc=deselect.');
    requestAnimationFrame(loop);
  }
  ...
  window.PMACustomRigNative = {
    ...
    suppressHugeNativeControllers: suppressAllHugeNativeCustomControllers,
